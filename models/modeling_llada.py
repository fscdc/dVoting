...
class RotaryEmbedding(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, past_q_pos: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        with torch.autocast(q.device.type, enabled=False):
            ...
            if past_q_pos is not None:
                ...
                q_ = self.apply_rotary_pos_emb(
                    torch.gather(pos_sin.repeat(B, 1, 1, 1), dim=2, index=past_q_pos), #pos_sin[past_q_pos], #[:, :, past_q_pos, :],
                    torch.gather(pos_cos.repeat(B, 1, 1, 1), dim=2, index=past_q_pos), #pos_cos[past_q_pos], #[:, :, past_q_pos, :],
                    q_,
                )
            else:
                q_ = self.apply_rotary_pos_emb(
    ...
    def attention(
        ...
        if self.config.rope:
            ...
            if transfer_idx is not None and self._q_cache:
                ...
                past_q_pos = None
            q, k = self.rotary_emb(q, k, past_q_pos)
        ...
        #    attn_weights = F.softmax(attention_scores / math.sqrt(q.size(-1)), dim=-1)
        #    import time
        #    torch.save({
        #        'attn': attn_weights, 
        #        'q': q,
        #        'k': k,
        #        'v': v,
        #    }, f'notebook/llada_qkv_20/qkvatt_{time.time()}_layer{self.layer_id}.pt')
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if layer_past is not None:
            ...
            past_k = past_k.transpose(1, 2).contiguous().view(
                past_k.size(0), past_k.size(2), self.config.d_model
            )
            past_v = past_v.transpose(1, 2).contiguous().view(
                past_v.size(0), past_v.size(2), self.config.d_model
            )

            # For Q
            if self._q_cache:
                unselected_x_normed = x_normed[~transfer_idx.unsqueeze(-1).expand_as(x_normed)].view(B, -1, D)
                q = self.q_proj(unselected_x_normed)
            else:
                q = self.q_proj(x_normed)

            # For Key
            if self._kv_cache:
                selected_k = past_k[transfer_idx.unsqueeze(-1).expand_as(past_k)].view(B, -1, D)
                start_time = time.time()
                unselected_x_normed = x_normed[~transfer_idx.unsqueeze(-1).expand_as(x_normed)].view(B, -1, D)
                start_time = time.time()
                k_proj = self.k_proj(unselected_x_normed)
                #print("k_proj:", time.time() - start_time, unselected_x_normed.shape)


                past_cur_k = torch.cat([selected_k, k_proj], dim=-2)

                reorder_idx_l = torch.cat([torch.nonzero(transfer_idx, as_tuple=True)[1], torch.nonzero(~transfer_idx, as_tuple=True)[1]], dim=0).view(B, -1)

                reorder_idx_l = reorder_idx_l.unsqueeze(-1).expand(-1, -1, past_cur_k.shape[-1])
                k = torch.zeros((B, L, D), device=past_cur_k.device, dtype=past_cur_k.dtype).scatter_(1, reorder_idx_l, past_cur_k)  #torch.scatter(past_cur_k, dim=1, index=reorder_idx_l)

                # for value
                selected_v = past_v[transfer_idx.unsqueeze(-1).expand_as(past_v)].view(B, -1, D)
                past_cur_v = torch.cat([selected_v, self.v_proj(unselected_x_normed)], dim=-2)
                v = torch.zeros((B, L, D), device=past_cur_v.device, dtype=past_cur_v.dtype).scatter_(1, reorder_idx_l, past_cur_v) #v = torch.gather(past_cur_v, dim=1, index=reorder_idx_l)
            ...
            start_time = time.time()
            k = self.k_proj(x_normed)
            #print("k time:", time.time() - start_time, x_normed.shape)
            #raise NotImplementedError
            v = self.v_proj(x_normed)
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, transfer_idx=transfer_idx)

        # Add attention scores.
        # shape: (B, T, C)

        if self._q_cache and layer_past is not None:
            q_pos_for_att = torch.nonzero(~transfer_idx, as_tuple=True)[1].view(B, -1, 1).expand(-1, -1, D)
            #print(att[0, 0], att.shape)
            att = torch.zeros((B, L, D), device=att.device, dtype=att.dtype).scatter_(1, q_pos_for_att, att) 
            #print(att, att[0, 49])
            #raise NotImplementedError
...
class LLaDAModel(nn.Module):
    ...
    def forward(
        ...
        # @sicheng: add for getting only last hidden state
        only_last_hidden_state: bool = False,
    ) -> LLaDAOutput:
        """
        ...
        # @sicheng: add for getting only last hidden state
        collect_all = output_hidden_states and (not only_last_hidden_state)
...
#
        ...
        if self.config.block_group_size == 1:
            for block_idx, block in enumerate(self.transformer.blocks):
                ...
                transfer_idx = None if past_query_key_values is None or not use_cache else past_query_key_values[1]
                if (
                    (self.activation_checkpointing_strategy == ActivationCheckpointingStrategy.whole_layer)
                    or (
                        self.activation_checkpointing_strategy == ActivationCheckpointingStrategy.one_in_two
                    ...
                    x, cache = self._activation_checkpoint_fn(
                        block, x, attention_bias=attention_bias, layer_past=layer_past, transfer_idx=transfer_idx, use_cache=use_cache,  block_idx=block_idx
                    )
                else:
                    # shape: (batch_size, seq_len, d_model)
                    x, cache = block(x, attention_bias=attention_bias, layer_past=layer_past, transfer_idx=transfer_idx, use_cache=use_cache, block_idx=block_idx)
                if attn_key_values is not None:
                    assert cache is not None
                    attn_key_values.append(cache)
        else:
            for group_idx, block_group in enumerate(self.transformer.block_groups):
                ...
                x, cache = block_group(
                    x, attention_bias=attention_bias, layers_past=layers_past, transfer_idx=transfer_idx, use_cache=use_cache
                )
                if attn_key_values is not None:
                    assert cache is not None
...
class LLaDAModelLM(PreTrainedModel):
    ...
    def forward(
        ...
        outputs = self.model.forward(
            ...
            only_last_hidden_state=only_last_hidden_state,
        )
