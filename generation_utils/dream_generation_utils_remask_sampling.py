...
def sample_tokens(logits, temperature=0.0, top_p=None, top_k=None, margin_confidence=False, neg_entropy=False, random=False):
...
class DreamGenerationConfig(GenerationConfig):
    def __init__(self, **kwargs):
        ...
        # @sicheng: add new params (can not pass the params here, but can pass into diffusion_generate)
        self.entropy_thresh = kwargs.pop("entropy_thresh", None)
        self.sampling_upper_bound = kwargs.pop("sampling_upper_bound", None)
...
class DreamGenerationMixin:
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        total_steps = result.steps if isinstance(result, DreamModelOutput) else None
    ...
    def _sample(
        ...
        **model_kwargs,
        ...
        # @sicheng: add for remask sampling
        entropy_thresh = generation_config.entropy_thresh
        sampling_upper_bound = generation_config.sampling_upper_bound
        block_size = generation_config.block_size
        print("[sicheng] entropy_thresh, sampling_upper_bound, block_size:", entropy_thresh, sampling_upper_bound, block_size)
        ...
        # @sicheng: batch size must be 1
        assert B == 1, "This remask sampling only supports batch_size=1 for now."

        gen_length = max_length - input_ids.shape[1]
        if block_size is None:
            block_size = gen_length  # Default: single block (original behavior)
        ...
        print("[sicheng] Start remask sampling generation...")

        # Process each block
        total_steps = 0
        for num_block in range(num_blocks):
            
            current_block_start = input_ids.shape[1] + num_block * block_size
            ...
            while True:
                ...
                for k in range(1, current_transfer_tokens):
                    # 只解码entropy低于entropy_thresh的tokens
                    if selected_entropy[0, k] < entropy_thresh:
