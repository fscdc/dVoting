...
class DreamGenerationMixin:
    ...
    def diffusion_generate(
        ...
        **kwargs,
        ...
        # @sicheng: generation_config changed at here
    ...
    def _sample(
        ...
        # @sicheng: add for semi-ar
        block_size = generation_config.block_size
        print("[sicheng] block_size:", block_size)
        ...
        if attention_mask is not None and torch.any(attention_mask == 0.0):
            # print("[!sicheng] use attention mask")
        ...
        # @sicheng: semi-ar sampling (block-wise, fixed steps)
        for num_block in range(num_blocks):
            ...
            for i in range(steps_per_block):
                ...
                # if current block is already finished, move to next block
                if (x[:, current_block_start:current_block_end] == mask_token_id).sum() == 0:
                    break

                # restrict decoding to current block only
                block_mask_index = mask_index.clone()
                block_mask_index[:, :current_block_start] = False
                block_mask_index[:, current_block_end:] = False
                ...
                if alg == 'origin':
                    ...
                    transfer_mask = torch.rand_like(x0.float()) < p_transfer
                    ...
                    # number of remaining tokens in current block
                    num_mask_token = block_mask_index.sum().item()
                    number_transfer_tokens = (
                        int(num_mask_token * (1 - s / t))
                        if i < steps_per_block - 1
                        else num_mask_token
                    )
                    number_transfer_tokens = max(1, number_transfer_tokens)
                    ...
                    if alg_temp is None or alg_temp == 0:
                        ...
                        scaled_conf = full_confidence / alg_temp
                        prob = F.softmax(scaled_conf, dim=-1)
                        transfer_index = torch.multinomial(
                            prob, num_samples=number_transfer_tokens
                        )
